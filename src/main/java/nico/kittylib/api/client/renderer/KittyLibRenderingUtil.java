...
public class KittyLibRenderingUtil {
    ...
    public static void renderQuad(MatrixStack.Entry entry, VertexConsumer buffer, double[] vertexes, float uMin, float vMin, float uMax, float vMax, int light, int overlay, int[] rgba) {
        // Front triangle
        assembleFace(
                new double[]{
        ...
        ).render(buffer, entry, light, overlay, rgba, new boolean[]{false, false, false});
        ...
        ).render(buffer, entry, light, overlay, rgba, new boolean[]{false, false, true});
    }
