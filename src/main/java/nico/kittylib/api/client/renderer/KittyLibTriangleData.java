...
public record KittyLibTriangleData(double[] vertex, double[] normal, float[] texturePos) {
    ...
    public void render(VertexConsumer buffer, Matrix4f positionMatrix, Matrix3f normalMatrix, int light, int overlay, int[] rgba, boolean[] mirror) {
        buffer
                .vertex(positionMatrix, (float) vertex[0] * (mirror[0]?-1:1), (float) vertex[1] * (mirror[1]?-1:1), (float) vertex[2] * (mirror[2]?-1:1))
                .color(rgba[0], rgba[1], rgba[2], rgba[3])
                .texture( texturePos[0], 1.0f - texturePos[1])
                .overlay(overlay)
                .light(light)
                .normal(normalMatrix, (float) normal[0], (float) normal[1], (float) normal[2])
                .next();
